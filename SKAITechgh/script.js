...
async function loadStoredOrders() {
    ...
    // Try fetching remote orders for authenticated agent
    try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
            const [dataOrdersRes, utilOrdersRes, airtimeOrdersRes] = await Promise.all([
                supabase.from('agent_data_orders').select('*').eq('agent_id', user.id).order('created_at', { ascending: false }).limit(50),
                supabase.from('utility_orders').select('*').eq('agent_id', user.id).order('created_at', { ascending: false }).limit(50),
                supabase.from('airtime_orders').select('*').or(`agent_id.eq.${user.id},user_id.eq.${user.id}`).order('created_at', { ascending: false }).limit(50)
            ]);

            const remoteOrders = [];
            if (dataOrdersRes?.data) {
                dataOrdersRes.data.forEach(o => {
                    remoteOrders.push({
            ...
            if (utilOrdersRes?.data) {
                utilOrdersRes.data.forEach(o => {
                    remoteOrders.push({
                        id: o.payment_reference || o.id.slice(0, 8),
                        network: (o.bill_type || '').toUpperCase(),
                        phone: o.customer_phone,
                        package: `${o.package_name || o.account_number} (GHS ${Number(o.amount).toFixed(2)})`,
                        status: (o.status === 'completed' || o.status === 'paid') ? 'Completed' : (o.status === 'failed' ? 'Canceled' : 'Pending'),
                        date: new Date(o.created_at).toLocaleDateString()
                    });
                });
            }
            if (airtimeOrdersRes?.data) {
                airtimeOrdersRes.data.forEach(o => {
                    const isDelivered = o.airtime_status === 'delivered';
                    const isFailed = o.airtime_status === 'failed' || o.payment_status === 'failed';
                    const status = isDelivered ? 'Completed' : (isFailed ? 'Canceled' : (o.payment_status === 'paid' ? 'Processing' : 'Pending'));
                    remoteOrders.push({
                        id: o.payment_reference || o.id.slice(0, 8),
        ...
        localAirtime.forEach(o => {
            if (o.reference && !existingIds.has(String(o.reference).toUpperCase())) {
                ...
                savedOrders.push({
                    ...
                    phone: o.phone,
                    package: `Airtime (GHS ${Number(o.amount).toFixed(2)})`,
                    status: isDelivered ? 'Completed' : (isFailed ? 'Canceled' : 'Processing'),
...
async function handleUtilityClick(event, service = 'ecg') {
    ...
    try {
        ...
        if (metaRole === 'agent' || metaRole === 'admin') {
            window.location.href = `utilityBills.html?service=${encodeURIComponent(service)}`;
            return false;
        }
