-- Manual MoMo/cashout settlement requests. Wallet credit is performed only by
-- the administrator review function below, after payment has been verified.
...
create policy "Agents can view their own cashout deposit requests"
  on public.cashout_deposit_requests for select using (auth.uid() = agent_id);
create policy "Agents can create their own cashout deposit requests"
  on public.cashout_deposit_requests for insert with check (auth.uid() = agent_id);
